...
import { GENDER_ROLES, MOBILE_GAME_ROLES, PC_GAME_ROLES, AGE_ROLES, DOMICILE_ROLES, BANNERS } from '../config/roles';
...
export function handleSetupRulesCommand(_interaction: DiscordInteraction): DiscordResponse {
  return {
    type: 4, // CHANNEL_MESSAGE_WITH_SOURCE
    data: {
      embeds: [
...
export function handleSetupAgeCommand(_interaction: DiscordInteraction): DiscordResponse {
...
export function handleSetupDomicileCommand(_interaction: DiscordInteraction): DiscordResponse {
      ...
      embeds: [
        {
          ...
          description: 
            ...
            DOMICILE_ROLES.map(role => `${role.emoji.name} | **${role.label}**`).join('\n'),
          color: 0x2ECC71, // Green
          image: {
